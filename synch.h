...
class Lock {
  public:
	int lockStatus ; 				// T11 To keep track of a lock's status. Can be 0 or 1. 0 -> FREE and 1 -> BUSY
	Thread* lockOwnerThread;			// T11 Keeps track of the thread which currently holds this lock
	List* lockWaitQueue;				// T11 The waiting queue for a lock
...
class Condition {
  public:
	Lock* waitingLock;	//Lock on which Condition-wait is called
	List* conditionWaitQueue; //Wait queue for condition wait
